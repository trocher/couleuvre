...
event SetWeights: # @audit-ok unused
...
# @audit-ok events no indexing at all.
...
# TODO placeholder
DYNAMIC_WEIGHT_EIP165_ID: constant(bytes4) = 0x12431234 # @audit-ok is that a placeholder or a real value?
# @audit-ok should be 0xa1aab33f nop ?
# receiver logic
...
def _is_dynamic(addr: address) -> bool:
    ...
    return success and convert(response, bool) or len(response) > 32 # @audit-ok how is that last check possible ?
...
def dispatch_fees(
    ...
    for r: Receiver in self.receivers:
        ...
        extcall crvusd.transfer(r.addr, balance * weight // MAX_BPS) # @audit-ok some small dust might remain because of rounding down for everyone
